...
backuptohome() {
  ...
  rsync -aczP -hi "$target" "$backup_path"
}
...
zipr(){
  zip -r "$(basename "$(pwd)")-$(date +%Y%m%d-%H%M%S).zip" . \
    -x "*.DS_Store*" "__MACOSX/*" "._*" "build/*" "install/*" "log/*" "*.zip"
}

unzipd() {
  unzip "$1" -d "$(basename "${1%.*}")"
}

z7a() {
  7zz a -mx=9 "$(basename "$(pwd)")-$(date +%Y%m%d-%H%M%S).7z" . \
    -x'!*.DS_Store' -x'!__MACOSX/*' -x'!._*' -x'!build/*' -x'!install/*' -x'!log/*' -x'!*.zip' -x'!*.7z'
}

z7x() {
  7zz x "$1" "-o$(basename "${1%.7z}")"
}
