...
# 用暂存区/索引区的该文件（即add后的文件）覆盖工作区
# staged/index --> workspace
git checkout <filename>
git restore <filename>

# 撤销提交到暂存区/索引区但是没有提交到本地仓库，即没有`git commit`的修改
# 用本地仓库的该文件（即最新commit的文件）覆盖暂存区/索引区
# repo/HEAD --> staged/index
git reset HEAD <filename>
# 再用暂存区/索引区的该文件（即仍是最新commit的文件）覆盖工作区
# staged/index --> workspace
git checkout <filename>
git restore <filename>
# 合并为一步（即用本地仓库的最新commit的文件覆盖工作区的该文件）
# repo/HEAD --> workspace
...
```sh
# git blame - 查看文件每行的修改历史（谁在什么时候修改了哪一行）
git blame <filename>
git blame -L 10,20 <filename>  # 只查看第10-20行
git blame -C <filename>        # 追踪代码移动/复制来源

# git bisect - 二分查找定位引入问题的提交
git bisect start
git bisect bad                 # 当前提交有问题
git bisect good <commit>       # 这个提交没问题
# git会自动切换到中间提交，测试后标记
git bisect good                # 当前提交没问题
git bisect bad                 # 当前提交有问题
# 找到问题提交后
git bisect reset               # 返回原来状态

# 简写方式
git bisect start HEAD <good-commit>
git bisect run <test-script>   # 自动运行脚本判断好坏
```
...
git submodule add -b <branch> <repo-url> [path]  # 指定分支
...
git submodule update --remote --merge  # 合并更新
...
# f, fixup [-C | -c] <commit> = like "squash" but keep only the previous commit message
...
# 1. 合并多个提交（squash）
# pick abc123 first commit
# squash def456 second commit  # 会被合并到前一个提交
# squash ghi789 third commit   # 会被合并到第一个提交

# 2. 修改历史提交信息（reword）
# pick abc123 first commit
# reword def456 second commit  # 会提示修改这个提交的信息

# 3. 编辑历史提交（edit）
# pick abc123 first commit
# edit def456 second commit    # 会在这里暂停，可以修改文件
# 修改完后 git add . && git rebase --continue

# 4. 删除提交（drop）
# pick abc123 first commit
# drop def456 second commit    # 这个提交会被删除
...
# 自动化 rebase
git rebase -i --autosquash   # 自动合并标记为 fixup/squash 的提交
# 配合 git commit --fixup=<commit> 使用
```
...
# 暂存工作现场
git stash
git stash push -m "description"  # 带描述信息

# 查看暂存列表
git stash list
# stash@{0}: On main: description
# stash@{1}: WIP on main: abc123 commit msg
...
git stash pop stash@{1}     # 恢复并删除指定暂存
...
git stash -a                # 包含未跟踪和忽略的文件
git stash --keep-index      # 只暂存未暂存的修改
```
...
git checkout --theirs <filename>   # 使用合并进来的版本
